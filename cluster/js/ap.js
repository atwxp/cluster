...
function init(edges /*nodePos*/, vetextNum, options) {    
    ...
    // 输出结果, 运行时间, 模块度
    console.log(ret, Date.now() - startTime, modularity(edges, num, options.edgeNum, ret));
    return ret;
}
...
function modularity(edges, num, edgeNum, ret) {
    var clubNum = ret.centerSet.length,
        club = ret.club, 
        clubItem,
        i, j, k, m, n, p,
        connect = [],
        adjM = initAdjM(edges, num),
        tre = 0, ai = 0, 
        temp;
    ...
    for (i = 0; i < clubNum; i++) {
        connect[i][i] = 0;
        clubItem = club[ret.centerSet[i]];
        for (j = 0; j < clubItem.length;  j++) {
            for (k = j + 1; k < clubItem.length; k++) {
                if (adjM[clubItem[j] - 1][clubItem[k] -1] == 1) {
                    connect[i][i]++;
                }
            }
        }

        connect[i][i] = connect[i][i] / edgeNum;
        tre += connect[i][i];

        for (m = i + 1; m < clubNum; m++) {
            connect[i][m] = 0;
            for (n = 0; n < clubItem.length; n++) {
                for (p = 0; p < club[ret.centerSet[m]].length; p++) {
                    if (adjM[clubItem[n] - 1][club[ret.centerSet[m]][p] -1] == 1) {
                        connect[i][m]++;
                    }
                }
            }
            connect[m][i] = connect[i][m] = connect[i][m] / edgeNum / 2;
        }
    }
